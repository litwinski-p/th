...
<div class="card" style="max-width: 560px; margin: 2rem auto;">
    ...
    <?php if (($errors['setup_token'] ?? null) !== null): ?>
        <div class="flash flash-error"><?= View::escape((string) $errors['setup_token']) ?></div>
    <?php endif; ?>

    <?php if (($canSetup ?? false) === true): ?>
        <form method="post" action="/setup" autocomplete="off" novalidate>
            <input type="hidden" name="_token" value="<?= View::escape(Csrf::token()) ?>">
            <input type="hidden" name="setup_token" value="<?= View::escape((string) ($setupToken ?? '')) ?>">
    ...
    <?php else: ?>
        <p class="text-muted">Provide the setup token in URL query string, then reload this page.</p>
    <?php endif; ?>
</div>
